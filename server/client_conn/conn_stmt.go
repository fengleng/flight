...
func (c *ClientConn) handleStmtPrepare(sql string) error {
	...
	n := c.schema.DefaultBackendNode

	//n := c.proxy.GetNode(defaultRule.Nodes[0])

	co, err := c.getWrapConn(n, false)
	defer c.closeShardConn(co, false)
	if err != nil {
...
func (c *ClientConn) handlePrepareSelect(stmt *sqlparser.Select, sql string, args []interface{}) error {
	defaultNode := c.schema.DefaultBackendNode
	...
	conn, err := c.getWrapConn(defaultNode, true)
	defer c.closeShardConn(conn, false)
	if err != nil {
		return err
	}
	...
	if err != nil {
		log.Error("ClientConn handlePrepareSelect %v %v", err.Error(), c.connectionId)
		return err
	}
...
func (c *ClientConn) handlePrepareExec(stmt sqlparser.Statement, sql string, args []interface{}) error {
	...
	conn, err := c.getWrapConn(defaultNode, false)
	defer c.closeShardConn(conn, false)
	if err != nil {
		return err
	}
	...
	if err != nil {
		log.Error("ClientConn handlePrepareExec %v %v", err.Error(), c.connectionId)
		return err
	}
